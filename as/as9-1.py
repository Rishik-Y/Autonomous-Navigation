...
# --- *** NEW: "PRO DRIVER" TUNING PARAMETERS *** ---
...
JERK_LIMIT = 1.0
LOAD_UNLOAD_TIME_S = 3.0 # <-- Restore loading time

# --- *** NEW: "PRO DRIVER" CONTROLLER PARAMETERS *** ---
LOOKAHEAD_GAIN = 1.2      # <-- Look further ahead for smoother turns
LOOKAHEAD_MIN_M = 4.0     
LOOKAHEAD_MAX_M = 35.0    # <-- Allow looking even further at high speed
...
def generate_curvy_path_from_nodes(node_list: list[np.ndarray], points_per_segment=20) -> list[np.ndarray]:
    ...
    # Silence the print, it's too noisy
...
class Car:
    def __init__(self, x_m, y_m, angle=0):
        ...
        self.op_state = "GOING_TO_ENDPOINT" # <-- State machine is back
        self.op_timer = 0.0
...
# --- *** MODIFIED: "PRO DRIVER" Main Simulation *** ---
def run_simulation():
    pygame.init()
    screen = pygame.display.set_mode((WIDTH, HEIGHT), pygame.RESIZABLE) 
    pygame.display.set_caption("Pro Driver: Dynamic A* with Smooth Braking")
    ...
    while running:
        ...
        if direction != 0:
            ...
            # === 2. SPEED (Curvature limit - unchanged, but uses new tuning) ===
            ...
            # === 3. NEW: "PRO DRIVER" SMOOTH BRAKING LOGIC ===
            ...
            BRAKING_START_DISTANCE_M = 30.0 # <-- Start braking 30m out
