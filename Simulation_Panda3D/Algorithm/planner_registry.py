...
def load_global_planner(planner_name=None):
    ...
    if not module_name:
        return None
    planner_class = _load_planner_class(f"Algorithm.global.{module_name}", GlobalPlannerInterface)
    return planner_class()
...
def load_local_planner(planner_name=None):
    ...
    if not module_name:
        return None
    planner_class = _load_planner_class(f"Algorithm.local.{module_name}", LocalPlannerInterface)
    return planner_class()
