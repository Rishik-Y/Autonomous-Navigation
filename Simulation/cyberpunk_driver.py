...
class CyberpunkDriver:
    ...
    def calculate_target_speed(self, my_car, other_cars, desired_speed):
        ...
        if critical_vehicle:
            # --- 3a. Deadlock Breaker (Head-On Priority) ---
            # Check if this is a head-on situation
            other_heading = np.array([math.cos(critical_vehicle.angle), math.sin(critical_vehicle.angle)])
            dot_heading = np.dot(my_heading, other_heading)
            
            # If facing opposite directions (dot < -0.8) AND in same lane (implied by critical_vehicle selection)
            if dot_heading < -0.8:
                # Resolve Deadlock by ID
                if my_car.id > critical_vehicle.id:
                    # I Yield (Stop)
                    return 0.0
                else:
                    # I Bully (Have Priority)
                    # We do NOT yield here. We proceed to standard safety checks.
                    # If the other car is far enough, we keep speed.
                    # If the other car is dangerously close, we MUST stop to avoid collision.
                    pass

            # --- 3b. Proportional Braking ---
            ...
            if min_dist <= self.min_standstill_distance_m:
                ...
                # Linear Interpolation (Lerp): Match Leader <-> Resume Speed
                # Alpha 0 (Close) -> lead_speed
                # Alpha 1 (Far)   -> desired_speed
                safe_speed = lead_speed + (desired_speed - lead_speed) * alpha
        ...
        # Refined: Only panic if the other car is actually threatening (in/near lane)
        if abs(my_car.steer_angle) > self.cornering_steer_threshold_rad:
             for other in vehicles_in_range:
                 ...
                 # Only slow down if they are CLOSE (< 10m) AND occupying our lane space (< 3m)
                 # This avoids braking for cars in safe parallel lanes
                 if dist < 10.0 and abs(lateral_dist) < 3.0:
