...
# --- NEW: Function to draw the entire road network graph ---
def draw_road_network(screen, g_to_s, scale):
    """Draws the entire road graph (all edges) as simple gray lines."""
    road_width_px = max(1, int(ROAD_WIDTH_M * METERS_TO_PIXELS * scale))
    for node1_name, node2_name in EDGES:
        p1_px = g_to_s(NODES[node1_name])
        p2_px = g_to_s(NODES[node2_name])
