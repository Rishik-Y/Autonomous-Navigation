...
# --- SIMULATION SETTINGS ---
NUM_PLATOONS = 2       # <-- REDUCED TO 2
TRUCKS_PER_PLATOON = 5 # 5 trucks per chain
...
# --- PLATOON MANAGEMENT ---
class Platoon:
    def __init__(self, road_graph, platoon_id):
        self.id = platoon_id
    ...
    def spawn(self):
        ...
        previous_truck = leader # The "Link"
        
        # 2. Followers (The Chain)
        for i in range(TRUCKS_PER_PLATOON - 1):
            # Spawn 12m behind previous truck using trig
            offset_x = -12.0 * math.cos(angle)
            offset_y = -12.0 * math.sin(angle)
            
            spawn_x = previous_truck.x_m + offset_x
            spawn_y = previous_truck.y_m + offset_y
            
            follower = Truck(spawn_x, spawn_y, angle=angle, start_node=start_node)
            follower.is_leader = False
            follower.front_truck = previous_truck # <--- THIS IS THE KEY: Follow the specific truck ahead
            follower.s_path_m = previous_truck.s_path_m - 12.0 # Estimate path position
            
            self.trucks.append(follower)
            previous_truck = follower # Move the link down the chain
        ...
        if leader.op_state != prev_op_state:
            ...
            if new_target:
                ...
                if route_nodes:
                    ...
                    # Force followers to reset their S tracking relative to leader
                    running_offset = 0.0
                    for t in self.trucks[1:]:
                        running_offset += 12.0
                        t.s_path_m = leader.s_path_m - (running_offset * direction)

        # --- FOLLOWER LOGIC ---
        for i, truck in enumerate(self.trucks):
            if i == 0: continue # Skip leader
            
            # Project onto the SHARED path
            f_pos = np.array([truck.kf.x[0], truck.kf.x[2]])
            truck.s_path_m = self.path.project(f_pos, truck.s_path_m)
            
            # State Sync: If leader is waiting/loading, followers must eventually stop too
            # We don't force state, but we let the ACC handle the stopping.
            # However, we can sync the op_state for display purposes
            truck.op_state = "PLATOON_FOLLOW" if leader.speed_ms > 0.1 else "PLATOON_WAIT"
...
def run_simulation():
    ...
    pygame.display.set_caption(f"Pro Driver: {NUM_PLATOONS} Platoons (Chain Logic)")
    ...
    while running:
        ...
        for platoon in platoons:
            ...
            for truck in platoon.trucks:
                ...
                # 2. Controller
                if truck.is_leader:
                    # Leader drives based on Path/State Machine
                    if direction != 0:
                        # Lookahead Speed Limit
                    ...
                    # Followers drive based on truck ahead
                    accel_cmd = truck.get_acc_control(dt)

                # 3. Steering (Everyone follows path)
                if direction != 0:
        ...
        for p in platoons: draw_active_path(screen, p.path, g_to_s, scale) 
        for p in platoons:
            for t in p.trucks: t.draw(screen, g_to_s)

        # HUD
        s = font.render(f"Platoons: {NUM_PLATOONS}", True, (0,0,0))
        screen.blit(s, (10, 10))
