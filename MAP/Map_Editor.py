...
def load_map_data():
    ...
    try:
        ...
        FUEL_ZONES = sandbox.get('FUEL_ZONES', [])
...
def handle_save_request(is_dirty: bool, cache_needs_regen: bool):
    was_dirty = is_dirty
    if save_map_data():
        if was_dirty:
            cache_needs_regen = True
        is_dirty = False
        return True, is_dirty, cache_needs_regen, "SAVED to Saved_Map/map_data.py"
    return False, is_dirty, cache_needs_regen, "ERROR saving map data"
...
def draw_road_network(screen, g_to_s, scale):
    ...
    for node_name, pos_m in NODES.items():
        ...
        elif node_name in FUEL_ZONES: color = ORANGE
            ...
            # --- Keyboard Input for Mode Change ---
                ...
                if allow_tab_switch and event.key == pygame.K_TAB:
                    ...
                    if is_dirty:
                        ...
                        if choice == "save":
                            _, is_dirty, cache_needs_regen, status_text = handle_save_request(is_dirty, cache_needs_regen)
                        elif choice == "cancel":
                            continue
                        elif choice == "quit":
                            running = False
                ...
                if event.key == pygame.K_s:
                    ...
                    mode, brush_color, status_text = 'add_orange', ORANGE, "Mode: ADD ORANGE (Fuel Zone)"
                    selection_start_node = None
                ...
                if event.button == 1: # Left Click
                    ...
                    if mode == 'manual':
                        ...
                        if not clicked_node:
                            ...
                            # Handle 'orange' -> 'fuel' naming convention if desired, or keep as 'orange'
                            if node_type == 'orange': node_type = 'fuel' 
                        ...
                        if clicked_node:
                            ...
                            if clicked_node in FUEL_ZONES: FUEL_ZONES.remove(clicked_node)
