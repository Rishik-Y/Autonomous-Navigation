...
NODES = {
    ...
    # --- *** NEW ***: SECOND STARTING ZONE ---
    "start_zone_2": np.array([680.0, 250.0]), # <--- New Base in the East (Region D)
}
...
EDGES = [
    ...
    ("fw_pit_1_b", "fw_load_spur_1"), ("fw_load_spur_1", "load_zone_4"),
    ("fw_pit_1_c", "fw_load_spur_2"), ("fw_load_spur_2", "load_zone_5"),
    ("fw_pit_1_d", "fw_load_spur_3"), ("fw_load_spur_3", "load_zone_6"),
    ("fw_pit_1_f", "fw_load_spur_4"), ("fw_load_spur_4", "load_zone_7"),
    ...
    ("fw_pit_2_b", "fw_load_spur_5"), ("fw_load_spur_5", "load_zone_8"),
    ("fw_pit_2_c", "fw_load_spur_6"), ("fw_load_spur_6", "load_zone_9"),
    ("fw_pit_2_d", "fw_load_spur_7"), ("fw_load_spur_7", "load_zone_10"),
    ("fw_pit_2_f", "fw_load_spur_8"), ("fw_load_spur_8", "load_zone_11"),
    ...
    ("fw_pit_3_b", "fw_load_spur_9"), ("fw_load_spur_9", "load_zone_12"),
    ("fw_pit_3_c", "fw_load_spur_10"), ("fw_load_spur_10", "load_zone_13"),
    ("fw_pit_3_d", "fw_load_spur_11"), ("fw_load_spur_11", "load_zone_14"),
    ("fw_pit_3_f", "fw_load_spur_12"), ("fw_load_spur_12", "load_zone_15"),
    ...
    ("n_q_1_b", "n_load_spur_1"), ("n_load_spur_1", "load_zone_16"),
    ("n_q_1_c", "n_load_spur_2"), ("n_load_spur_2", "load_zone_17"),
    ("n_q_1_e", "n_load_spur_3"), ("n_load_spur_3", "load_zone_18"),
    ("n_q_1_a", "n_q_2_a"), ("n_q_2_a", "n_q_2_b"), ("n_q_2_b", "n_q_2_c"), ("n_q_2_c", "n_q_2_d"), ("n_q_2_d", "n_q_2_e"), ("n_q_2_e", "n_q_2_f"), ("n_q_2_f", "n_q_1_f"),
    ("n_q_2_b", "n_load_spur_4"), ("n_load_spur_4", "load_zone_19"),
    ("n_q_2_c", "n_load_spur_5"), ("n_load_spur_5", "load_zone_20"),
    ("n_q_2_e", "n_load_spur_6"), ("n_load_spur_6", "load_zone_21"),
    ...
    ("e_grid_1_d", "e_connector_n"), ("e_grid_2_d", "e_connector_n"),
    ("e_grid_3_d", "e_connector_s"), ("e_grid_4_d", "e_connector_s"),
    ...
    ("s_sp_1_b", "s_dump_spur_1"), ("s_dump_spur_1", "dump_zone_15"),
    ("s_sp_1_c", "s_dump_spur_2"), ("s_dump_spur_2", "dump_zone_16"),
    ("s_sp_1_e", "s_dump_spur_3"), ("s_dump_spur_3", "dump_zone_17"),
    ...
    ("s_sp_2_b", "s_dump_spur_4"), ("s_dump_spur_4", "dump_zone_18"),
    ("s_sp_2_d", "s_dump_spur_5"), ("s_dump_spur_5", "dump_zone_19"),
    ("s_sp_2_e", "s_dump_spur_6"), ("s_dump_spur_6", "dump_zone_20"),
    ("s_sp_2_g", "s_dump_spur_7"), ("s_dump_spur_7", "dump_zone_21"),
    ...
    ("sw_hub", "sw_dump_spur_1"), ("sw_dump_spur_1", "dump_zone_22"),
    ("sw_hub", "sw_dump_spur_2"), ("sw_dump_spur_2", "dump_zone_23"),
    ("sw_hub", "sw_dump_spur_3"), ("sw_dump_spur_3", "dump_zone_24"),
    ("sw_hub", "sw_load_spur_1"), ("sw_load_spur_1", "load_zone_22"),
    ("sw_hub", "sw_load_spur_2"), ("sw_load_spur_2", "load_zone_23"),
    ("sw_haul_2", "sw_connector_2"), ("sw_connector_2", "sw_haul_3"),
    ("sw_hub", "sw_connector_1"), ("sw_connector_1", "sw_haul_3"),
    ("s_connector_2", "sw_hub"), # Connects Region E and F

    # --- *** NEW ***: SECOND STARTING ZONE CONNECTION ---
    ("start_zone_2", "e_hub"), # <--- Connects new base
]

# --- Zone Definitions (Mega-Map) ---
LOAD_ZONES = [
...
DUMP_ZONES = [
    "dump_zone_1", "dump_zone_2", "start_zone", # Original
    "start_zone_2", # <--- *** NEW ***: Add new base as a dump/return zone
...
# --- Visual Road Chains for Spline Drawing (Mega-Map) ---
VISUAL_ROAD_CHAINS = [
    ...
    # --- *** NEW ***: SECOND STARTING ZONE DRAWING ---
    ["start_zone_2", "e_hub"], # <--- Draw new base road
]
...
# --- *** NEW ***: REFACTORED CAR CLASS ---
class Car:
    def __init__(self, start_node_name: str, road_graph: dict, nodes_dict: dict):
        # 1. Set Start Position
        start_pos = nodes_dict[start_node_name]
        self.x_m = start_pos[0]
        self.y_m = start_pos[1]
        
        # 2. Store graph and node info
        self.road_graph = road_graph
        self.nodes_dict = nodes_dict

        # 3. Calculate Initial Path
        self.current_node_name = start_node_name
        self.target_node_name = random.choice(LOAD_ZONES)
        print(f"Spawning Car at '{self.current_node_name}' -> Headed to '{self.target_node_name}'")
        
        route_nodes = a_star_pathfinding(self.road_graph, self.nodes_dict, self.current_node_name, self.target_node_name)
        if not route_nodes:
            self.path = Path([start_pos, start_pos + np.array([1,0])]) # Fallback
        else:
            waypoints = generate_curvy_path_from_nodes(route_nodes)
            self.path = Path(waypoints)

        # 4. Physics & State
        if len(self.path.wp) > 1:
            self.angle = math.atan2(self.path.wp[1][1] - self.path.wp[0][1], self.path.wp[1][0] - self.path.wp[0][0])
        else:
            self.angle = 0.0
        ...
        self.a_cmd_prev = 0.0
        self.s_path_m = 0.0
        self.desired_speed_ms = 0.0 # For HUD
        
        # 5. *** NEW ***: Each car has its own Kalman Filter
        self.kf = KalmanFilter(dt=1.0/60.0)
        self.kf.x = np.array([self.x_m, 0, self.y_m, 0])

    def move(self, accel_cmd, steer_input, dt):
        # (Physics code is unchanged from original)
    ...
    def _update_op_state(self, dt, path_length_m, current_s_m):
        # (State machine logic is unchanged from original)
        prev_op_state = self.op_state
        direction = 0
        base_speed_ms = 0
        ...
        if self.op_state == "GOING_TO_ENDPOINT": # Going to Load
            ...
            direction, base_speed_ms = +1, SPEED_MS_EMPTY
            ...
            direction, base_speed_ms = -1, SPEED_MS_LOADED
        elif self.op_state == "UNLOADING":
            if self.op_timer > 0: self.op_timer -= dt
            else: self.op_state = "GOING_TO_ENDPOINT"; self.current_mass_kg = MASS_KG
            direction, base_speed_ms = 0, 0.0
        
        return direction, base_speed_ms, self.op_state != prev_op_state
    ...
    def draw(self, screen, g_to_s):
        # (Drawing code is unchanged from original)
    ...
    def update_mission_and_physics(self, dt):
        """This is the main update function to call every frame."""
        
        # --- Step 1: Get State from KF ---
        est_pos_m = np.array([self.kf.x[0], self.kf.x[2]])
        est_vel_m = np.array([self.kf.x[1], self.kf.x[3]])
        est_speed_ms = np.linalg.norm(est_vel_m)
        self.s_path_m = self.path.project(est_pos_m, self.s_path_m) 

        # --- Step 2: Update State Machine ---
        direction, base_speed_ms, state_changed = self._update_op_state(dt, self.path.length, self.s_path_m)

        # --- Step 3: Dynamic Path Re-planning (if state changed) ---
        if state_changed:
            print(f"Car at {self.current_node_name} state change -> {self.op_state}")
            est_pos_m = np.array([self.kf.x[0], self.kf.x[2]]) # Get fresh estimate
            if self.op_state == "RETURNING_TO_START": # Finished Loading
                self.current_node_name = self.target_node_name
                self.target_node_name = random.choice(DUMP_ZONES)
                print(f"A* re-planning: from '{self.current_node_name}' to '{self.target_node_name}'")
                route_nodes = a_star_pathfinding(self.road_graph, self.nodes_dict, self.current_node_name, self.target_node_name)
                waypoints_m = generate_curvy_path_from_nodes(list(reversed(route_nodes)))
                self.path = Path(waypoints_m)
                self.s_path_m = self.path.project(est_pos_m, self.path.length) # Project to end of new path
            
            elif self.op_state == "GOING_TO_ENDPOINT": # Finished Dumping
                self.current_node_name = self.target_node_name
                self.target_node_name = random.choice(LOAD_ZONES)
                print(f"A* re-planning: from '{self.current_node_name}' to '{self.target_node_name}'")
                route_nodes = a_star_pathfinding(self.road_graph, self.nodes_dict, self.current_node_name, self.target_node_name)
                ...
                self.path = Path(waypoints_m)
                self.s_path_m = self.path.project(est_pos_m, 0.0) # Project to start of new path
        ...
        if direction != 0:
            ...
            s_target_steer = self.s_path_m + direction * ld
            p_target = self.path.point_at(s_target_steer)
            dx_local = (p_target[0] - est_pos_m[0]) * math.cos(self.angle) + (p_target[1] - est_pos_m[1]) * math.sin(self.angle)
            dy_local = -(p_target[0] - est_pos_m[0]) * math.sin(self.angle) + (p_target[1] - est_pos_m[1]) * math.cos(self.angle)
            ...
            # 2. SPEED (Curvature + Speed Floor)
            max_curvature = 0.0
            for step in np.linspace(0, 15.0, 10):
                curvature = self.path.get_curvature_at(self.s_path_m + direction * step)
                max_curvature = max(max_curvature, abs(curvature))
            
            if max_curvature > 1e-4: 
                v_turn_cap_raw = math.sqrt(MAX_LAT_ACCEL / max_curvature)
                v_turn_cap = max(v_turn_cap_raw, 2.5) # 2.5 m/s speed floor
            else: 
                v_turn_cap = base_speed_ms
            
            # 3. SMOOTH BRAKING
            target_s = 0.0 if direction == -1 else path_length_m
            dist_to_target = abs(target_s - self.s_path_m)
            ...
            # 4. FINAL SPEED
            self.desired_speed_ms = min(base_speed_ms, v_turn_cap, v_profile_cap, v_panic_cap)
            speed_error_ms = self.desired_speed_ms - est_speed_ms
        ...
        self.move(accel_cmd, steer_input, dt)
        accel_vec_m = np.array([self.accel_ms2 * math.cos(self.angle), self.accel_ms2 * math.sin(self.angle)])
        self.kf.predict(u=accel_vec_m)
        self.kf.update(z=self.get_noisy_measurement())
...
def run_simulation():
    ...
    # --- Step 1: Build Graph & Pre-Calculate Splines (Unchanged) ---
    ...
    # --- *** NEW ***: Setup Multiple Cars ---
    cars = []
    cars.append(Car("start_zone", road_graph, NODES))   # Car 1
    cars.append(Car("start_zone_2", road_graph, NODES)) # Car 2
    ...
    while running:
        ...
        # --- *** NEW ***: Main Update and Drawing Loop ---
        ...
        # --- Step 2: Update and Draw Each Car ---
        for car in cars:
            # Update physics, KF, and mission state
            car.update_mission_and_physics(dt)
            
            # Draw this car's active path
            draw_active_path(screen, car.path, g_to_s, scale)
            
            # Draw the car
            car.draw(screen, g_to_s)
            
            # Draw its KF estimate
            kf_pos_m = (car.kf.x[0], car.kf.x[2]); kf_screen = g_to_s(kf_pos_m)
            pygame.draw.circle(screen, RED, kf_screen, 6, 2)

        # --- Step 3: HUD (Displaying info for the FIRST car) ---
        if cars: # Only show HUD if at least one car exists
            car_to_track = cars[0] 
            speed_kmph = car_to_track.speed_ms * 3.6
            hud_texts = [
                f"[TRUCK 0] Speed: {speed_kmph:.1f} km/h (Target: {car_to_track.desired_speed_ms*3.6:.1f})",
                f"[TRUCK 0] Mass: {car_to_track.current_mass_kg:.0f} kg", f"State: {car_to_track.op_state}",
                f"[TRUCK 0] Target: {car_to_track.target_node_name}", 
                f"Path: {car_to_track.s_path_m:.1f}m / {car_to_track.path.length:.1f}m"
            ]
...
if __name__ == '__main__':
    print("Starting 'Pro Driver' MULTI-TRUCK simulation...")
    run_simulation()