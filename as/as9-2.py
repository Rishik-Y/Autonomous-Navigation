...
NODES = {
    ...
    # --- *** THIS IS THE FIX *** ---
    # The quotes "" are gone and replaced with brackets []
...
def run_simulation():
    ...
    # --- Setup Car, KF, and View (Unchanged) ---
    ...
    while running:
        ...
        if new_op_state != prev_op_state:
            ...
            if new_op_state == "RETURNING_TO_START": # Finished Loading
                car.current_node_name = car.target_node_name; car_target_node = random.choice(DUMP_ZONES); car.target_node_name = car_target_node
                ...
                car.current_node_name = car.target_node_name; car_target_node = random.choice(LOAD_ZONES); car.target_node_name = car_target_node
                print(f"A* re-planning: from '{car.current_node_name}' to '{car_target_node}'")
                route_nodes = a_star_pathfinding(road_graph, NODES, car.current_node_name, car_target_node)
                waypoints_m = generate_curvy_path_from_nodes(route_nodes); current_path = Path(waypoints_m)
