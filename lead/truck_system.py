...
class Truck:
    def __init__(self, x_m, y_m, angle=0, start_node="start_zone"):
        ...
        # Platoon Identity
        self.is_leader = True
        self.leader_truck = None # If I am a follower, who is my leader?
        self.front_truck = None  # Who is immediately in front of me?
    ...
    # --- NEW: FOLLOWER LOGIC (Adaptive Cruise Control) ---
    def get_acc_control(self, dt):
        if not self.front_truck: return 0.0 # Safety
        
        # Calculate gap
        my_pos = np.array([self.x_m, self.y_m])
        front_pos = np.array([self.front_truck.x_m, self.front_truck.y_m])
        dist_to_front = np.linalg.norm(front_pos - my_pos)
        
        # Target Gap (Safety distance)
        SAFE_GAP_M = 12.0 
        
        # Simple P-Controller for gap maintenance
        gap_error = dist_to_front - SAFE_GAP_M
        
        # Feed Forward: Match the front car's speed
        target_speed = self.front_truck.speed_ms
        
        # Correction: If too far, speed up. If too close, slow down.
        # Kp = 0.5 (Gain)
        correction_speed = gap_error * 0.8
        
        final_target_speed = target_speed + correction_speed
        final_target_speed = max(0.0, final_target_speed) # Can't go reverse
        
        speed_error = final_target_speed - self.speed_ms
        accel_cmd = np.clip(speed_error / 0.4, -MAX_BRAKE_DECEL, MAX_ACCEL_CMD)
        
        # E-STOP if extremely close
    ...
    def update_op_state(self, dt, path_length_m, current_s_m):
        # Simplified for followers: If following, just stay in "GOING" state
        if not self.is_leader:
             self.op_state = "PLATOONING"
    ...
    def draw(self, screen, g_to_s):
        ...
        # Color logic: Leaders are Blue/Grey, Followers are slightly greener
        if self.is_leader:
            ...
            body_color = (0, 150, 100) # Greenish for followers
