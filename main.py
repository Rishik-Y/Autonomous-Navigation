...
import heapq
import random
from truck_system import Truck, Path  # <--- IMPORTS YOUR NEW TRUCK SYSTEM
...
# --- CONTROLLER TUNING (The "Brain" of the driver) ---
LOOKAHEAD_GAIN = 1.2
LOOKAHEAD_MIN_M = 4.0
...
NODES = {
    ...
    # --- REGION G: NORTH-EAST QUARRY (LOAD) ---
    ...
    # --- REGION H: SOUTHERN SERVICE BAY (PARK/FUEL) ---
...
# --- HELPER FUNCTIONS (Map & Path Generation) ---
...
def run_simulation():
    ...
    pygame.display.set_caption("Pro Driver: Split File Architecture")
    clock = pygame.time.Clock()
    font = pygame.font.SysFont("Consolas", 18)

    # 1. Build Graph & Visuals
    ...
    # 2. Setup Single Truck
    car_start_node = "parking_1" 
    ...
    initial_pos = NODES[car_start_node]
    initial_angle = math.atan2(waypoints_m[1][1] - waypoints_m[0][1], waypoints_m[1][0] - waypoints_m[0][0])
    
    # INSTANTIATE TRUCK FROM IMPORTED CLASS
    car = Truck(initial_pos[0], initial_pos[1], angle=initial_angle, start_node=car_start_node)
    ...
    while running:
        ...
        # --- State Estimation ---
        # Uses the KF built into the Truck class now
        est_pos_m = np.array([car.kf.x[0], car.kf.x[2]])
        est_vel_m = np.array([car.kf.x[1], car.kf.x[3]])
        ...
        if new_op_state != prev_op_state:
            ...
            if new_op_state == "RETURNING_TO_START": # Go to Dump/Park
            ...
            elif new_op_state == "GOING_TO_ENDPOINT": # Go to Load
                car.current_node_name = car.target_node_name
                car_target_node = random.choice(LOAD_ZONES) 
                car.target_node_name = car_target_node
                route_nodes = a_star_pathfinding(road_graph, NODES, car.current_node_name, car_target_node)
        ...
        # --- Controller Logic (The Driver's Brain) ---
        if direction != 0:
            # 1. Steering
            ...
            # Use the constants from Truck system for Wheelbase if possible, 
            # but for now we use the imported class attribute or hardcode if needed.
            # Using car.WHEELBASE_M would be ideal if we exposed it, but we know it's 2.8
            steer_input = math.atan2(2.0 * 2.8 * math.sin(alpha), ld)
            
            # 2. Speed limit based on curvature
            ...
            steer_input = 0.0
            accel_cmd = np.clip((0.0 - est_speed_ms) / max(0.4, dt), -MAX_BRAKE_DECEL, MAX_ACCEL_CMD)
            desired_speed_ms = 0.0 
        ...
        car.draw(screen, g_to_s) # Truck draws itself now

        # HUD
        speed_kmph = car.speed_ms * 3.6
        hud_texts = [
            f"Speed: {speed_kmph:.1f} km/h (Target: {desired_speed_ms*3.6:.1f})",
            f"Mass: {car.current_mass_kg:.0f} kg", f"State: {car.op_state}", 
            f"Target: {car.target_node_name}", 
            f"Dist: {path_length_m - est_s_path_m:.1f}m"
        ]
