...
METERS_TO_PIXELS = 3.0 # Needed for drawing inside the class
CAR_COLOR = (0, 80, 200)

# --- TUNING ---
STEER_MAX_RAD = math.radians(STEER_MAX_DEG)
STEER_RATE_DEGPS = 720.0
...
class KalmanFilter:
    def __init__(self, dt, start_x, start_y):
        ...
        self.Q = np.eye(4) * 0.1
        self.R = np.eye(2) * (0.5**2) # 0.5 is Sensor noise std dev
        self.P = np.eye(4)
...
class Truck:
    ...
    def move(self, accel_cmd, steer_input, dt):
        ...
        F_brake_force = brake_force_from_command(brake, self.current_mass_kg)
        ...
        self.kf.predict(accel_vec)
        self.kf.update(self.get_noisy_measurement())
    ...
    def get_noisy_measurement(self):
        # 0.5 is Noise STD DEV
        return np.array([self.x_m + np.random.normal(0, 0.5),
                         self.y_m + np.random.normal(0, 0.5)])

    def draw(self, screen, g_to_s):
        # Calculate pixels based on the g_to_s scale
        ...
        # Draw KF ghost
        kf_pos = g_to_s((self.kf.x[0], self.kf.x[2]))
        pygame.draw.circle(screen, (255,0,0), kf_pos, 6, 2)