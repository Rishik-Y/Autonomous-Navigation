...
ZOOM_FACTOR = 1.1
PADDING = 50
# Using the conversion from try12.py as it's part of the "perfect" truck logic
...
# --- HELPER FUNCTIONS (from MAP/chain/main.py) ---
...
def run_simulation():
    ...
    pygame.display.set_caption("Pro Trucker on Procedural Map")
    ...
    print(f"Loading pre-computed map data from '{cache_filename}'...")
    with open(cache_filename, 'rb') as f:
        cache_data = pickle.load(f)
    
    road_graph = cache_data['road_graph']
    PRE_CALCULATED_SPLINES[:] = cache_data['pre_calculated_splines']
    ...
    # Generate the smooth, drivable waypoints from the A* node path
    waypoints_m = map_data.generate_curvy_path_from_nodes(route_nodes)
    ...
    while running:
        ...
        if direction == 0 and car.op_timer <= 0: # Time to get a new path
            ...
            route_nodes = a_star_pathfinding(road_graph, map_data.NODES, car.current_node_name, new_target)
            if route_nodes:
                waypoints_m = map_data.generate_curvy_path_from_nodes(route_nodes)
                path = Path(waypoints_m)
