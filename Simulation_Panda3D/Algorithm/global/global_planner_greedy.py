...
from Algorithm.planner_interface import GlobalPlannerInterface
...
class Planner(GlobalPlannerInterface):
    ...
    def optimize_assignments(self, trucks, site_states):
        ...
        for truck in free_trucks:
            if truck.current_node_name in map_data.NODES:
                start = truck.current_node_name
            else:
                start = DEFAULT_START_NODE
                print(
                    f"GlobalGreedyPlanner: Truck {truck.id} missing node, defaulting to {DEFAULT_START_NODE}."
                )
            best_mine = None
            best_time = float("inf")
