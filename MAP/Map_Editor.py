...
FUEL_ZONES = []
VISUAL_ROAD_CHAINS = []
MAP_DATA_FALLBACK_PATH = map_storage.legacy_path('map_data.py')

# --- HELPER FUNCTIONS ---
def load_map_data():
    global NODES, EDGES, LOAD_ZONES, DUMP_ZONES, FUEL_ZONES, VISUAL_ROAD_CHAINS
    map_file = map_storage.resolve_input_path('map_data.py', [MAP_DATA_FALLBACK_PATH])
...
def save_map_data():
    ...
    try:
        ...
        map_storage.write_text_file(
            "map_data.py",
            output.getvalue(),
            copy_targets=[MAP_DATA_FALLBACK_PATH]
        )
...
def fix_intersections():
    ...
    # --- Main Editor Loop ---
def run_editor(mode_label="Map Editor", allow_tab_switch=False):
    global NODES, EDGES, LOAD_ZONES, DUMP_ZONES, FUEL_ZONES, VISUAL_ROAD_CHAINS
    pygame.init()
