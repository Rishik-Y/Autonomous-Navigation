...
NODES = {
    ...
    "purple_auto_1": np.array([216.1, 262.6]),
    "purple_auto_2": np.array([236.2, 260.1]),
...
VISUAL_ROAD_CHAINS = [
    ...
    ('service_loop_5', 'fuel_1'),
    ('service_loop_5', 'fuel_2'),
    ['ix_east_2', 'purple_auto_1', 'purple_auto_2'],
    ['connector_2_2', 'purple_auto_2'],
]
...
if __name__ == '__main__':
    ...
    def generate_curvy_path_from_nodes(node_list: list[np.ndarray], points_per_segment=20) -> list[np.ndarray]:
        ...
        # Pad the node list to handle endpoints correctly for the spline
        node_list_padded = [node_list[0]] + node_list + [node_list[-1]]

        for i in range(len(node_list_padded) - 3):
            p0 = node_list_padded[i]
            p1 = node_list_padded[i + 1]
            p2 = node_list_padded[i + 2]
            p3 = node_list_padded[i + 3]
    ...
    print("Generating map cache...")

    # 1. Build road_graph
    road_graph = {name: [] for name in NODES}
    for node1_name, node2_name in EDGES:
        if node1_name in NODES and node2_name in NODES:
            p1 = NODES[node1_name]
            p2 = NODES[node2_name]
            distance = np.linalg.norm(p1 - p2)
            road_graph[node1_name].append((node2_name, distance))
            road_graph[node2_name].append((node1_name, distance))

    # 2. Build pre_calculated_splines
    pre_calculated_splines = []
    for chain in VISUAL_ROAD_CHAINS:
        chain_as_list = list(chain)
        node_coords = [NODES[name] for name in chain_as_list if name in NODES]
        if len(node_coords) >= 2:
            spline_points = generate_curvy_path_from_nodes(node_coords)
            pre_calculated_splines.append(spline_points)

    # 3. Create cache data and pickle it
    cache_data = {
        'road_graph': road_graph,
        'pre_calculated_splines': pre_calculated_splines,
    }

    with open('map_cache.pkl', 'wb') as f:
        pickle.dump(cache_data, f)

    print("Successfully generated map_cache.pkl")