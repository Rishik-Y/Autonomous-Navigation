...
# --- Constants ---
# --- Main Settings ---
IMAGE_FILE = os.path.join(os.path.dirname(__file__), 'try_with_car.png')
WAYPOINT_STEP_SIZE = 10     # Distance between waypoints. Larger = smoother but less accurate path.
SMOOTH_ITERATIONS = 5       # How many times to smooth the path. More = more rounded corners.
...
SPEED_STRAIGHT_PXPS = 250.0  # Pixels Per Second on straight sections
SPEED_CURVE_PXPS = 150.0     # Pixels Per Second on curves
MAX_ACCELERATION_PXPS = 150.0
MAX_STEER_ANGLE = math.pi / 4
CAR_LENGTH_PX = 40.0
CAR_WIDTH_PX = 20.0
WAYPOINT_THRESHOLD_PX = 40.0 # How close to a waypoint to switch to the next

# --- Controller Look-ahead ---
LOOK_AHEAD_STEERING_IDX = 4  # Aim N waypoints ahead for steering
LOOK_AHEAD_SPEED_IDX = 8     # Check N waypoints ahead for curves
CURVE_THRESHOLD_DOT = 0.97   # Dot product threshold to detect a "straight" line
...
if __name__ == '__main__':
    ...
    if not os.path.exists(IMAGE_FILE):
        ...
        # --- Step 3: Run Simulation if Waypoints were Found ---
        if final_waypoints and len(final_waypoints) > 1:
            print("Waypoints successfully generated. Starting Pygame simulation...")
            run_simulation(final_waypoints)
        else:
            print("Failed to generate a valid path from the image. Exiting.")