...
NODES = {
    ...
    "service_exit": np.array([150.0, 280.0]),
    "main_hub":     np.array([150.0, 250.0]),

    # --- Main "Highway" Intersections ---
    "ix_west":      np.array([100.0, 250.0]),
    "ix_east":      np.array([200.0, 250.0]),
    "ix_north":     np.array([150.0, 200.0]),

    # --- Loading District (Top-Left) ---
    "load_hub_1":   np.array([100.0, 200.0]),
    "load_spur_1a": np.array([80.0, 180.0]),
    "load_zone_1":  np.array([80.0, 160.0]), # <-- LOAD 1
    
    "load_spur_1b": np.array([120.0, 180.0]),
    "load_zone_2":  np.array([120.0, 160.0]), # <-- LOAD 2
    
    # --- Loading District (Top-Right) ---
    "load_hub_2":   np.array([200.0, 200.0]),
    "load_spur_2a": np.array([180.0, 150.0]),
    "load_spur_2b": np.array([220.0, 150.0]),
    "load_zone_3":  np.array([200.0, 120.0]), # <-- LOAD 3

    # --- Dumping District (Bottom-Right) ---
    "dump_hub_1":   np.array([200.0, 280.0]),
    "dump_spur_1a": np.array([180.0, 320.0]),
    "dump_zone_1":  np.array([180.0, 340.0]), # <-- DUMP 1
    "dump_spur_1b": np.array([220.0, 320.0]),
    "dump_zone_2":  np.array([220.0, 340.0]), # <-- DUMP 2
    
    # --- Redundant Connector Roads ---
    "connector_1":  np.array([100.0, 100.0]), # Connects load_zone_3 to load_hub_1
    "connector_2":  np.array([250.0, 250.0]), # Connects ix_east to dump_zone_2
}

# 2. Define the connections (edges)
EDGES = [
    # Start Area
    ("start_zone", "service_exit"),
    ("service_exit", "main_hub"),

    # Main Hub Connections
    ("main_hub", "ix_west"),
    ("main_hub", "ix_east"),
    ("main_hub", "ix_north"),
    ("main_hub", "dump_hub_1"), # Direct path to dumping
    ...
    ("load_hub_1", "load_spur_1a"),
    ("load_spur_1a", "load_zone_1"),
    ("load_hub_1", "load_spur_1b"),
    ("load_spur_1b", "load_zone_2"),

    # Loading Spur 2 (from load_hub_2)
    ("load_hub_2", "load_spur_2a"),
    ("load_hub_2", "load_spur_2b"),
    ("load_spur_2a", "load_zone_3"),
    ("load_spur_2b", "load_zone_3"),

    # Dumping Spur (from dump_hub_1)
    ("dump_hub_1", "dump_spur_1a"),
    ("dump_spur_1a", "dump_zone_1"),
    ("dump_hub_1", "dump_spur_1b"),
    ("dump_spur_1b", "dump_zone_2"),

    # Redundant Connectors
    ("load_zone_3", "connector_1"),
    ("connector_1", "load_hub_1"), # <-- Creates a big loop!
    ("connector_2", "dump_zone_2"), # <-- Creates a shortcut!
]
