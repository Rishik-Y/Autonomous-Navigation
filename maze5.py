...
TARGET_SPEED_GUPS = 3.0  # Grid Units Per Second (DEPRECATED, see below)
SPEED_STRAIGHT_GUPS = 3.0  # 30 km/h equivalent
# SPEED_CURVE_GUPS = (4.0 * (25.0 / 30.0)) # 25 km/h equivalent (~3.33)
SPEED_CURVE_GUPS = 2
...
# --- NEW: Controller Look-ahead constants ---
...
def main():
    ...
    while running:
        ...
        # --- Navigation: Waypoint Progress ---
        # This part just advances the index. We aim for the *next* point.
        progress_target_grid = waypoints_grid[current_waypoint_idx]
        dist_to_progress_target = np.linalg.norm(progress_target_grid - est_pos_grid)
        ...
        # --- Speed Control (Your Request) ---
        # Look ahead to see if the path is curved
        if direction == 1:
            speed_look_ahead_idx = min(current_waypoint_idx + LOOK_AHEAD_SPEED_IDX, len(waypoints_grid) - 1)
            next_point_idx = min(current_waypoint_idx + 1, len(waypoints_grid) - 1)
        else:
            speed_look_ahead_idx = max(current_waypoint_idx - LOOK_AHEAD_SPEED_IDX, 0)
            next_point_idx = max(current_waypoint_idx - 1, 0)

        p_car = est_pos_grid
        p_next = waypoints_grid[next_point_idx]
        p_future = waypoints_grid[speed_look_ahead_idx]

        v1 = p_next - p_car
        v2 = p_future - p_next

        # Check for zero-length vectors (e.g., at the end of a path)
        if np.linalg.norm(v1) < 0.1 or np.linalg.norm(v2) < 0.1:
            is_straight = True # Default to straight if we can't calculate
        else:
            norm_v1 = v1 / np.linalg.norm(v1)
            norm_v2 = v2 / np.linalg.norm(v2)
            dot_product = np.dot(norm_v1, norm_v2)
            is_straight = dot_product > CURVE_THRESHOLD_DOT # e.g., > 0.98

        # Set desired speed based on user's request
        if is_straight:
            desired_speed_gups = SPEED_STRAIGHT_GUPS # 30 km/h equivalent
        else:
            desired_speed_gups = SPEED_CURVE_GUPS   # 25 km/h equivalent
        ...
        # --- Steering Control (With Look-ahead) ---
        # This part aims for a point *further down the path*
        if direction == 1:
            steering_target_idx = min(current_waypoint_idx + LOOK_AHEAD_STEERING_IDX, len(waypoints_grid) - 1)
        else:
            steering_target_idx = max(current_waypoint_idx - LOOK_AHEAD_STEERING_IDX, 0)

        steering_target_grid = waypoints_grid[steering_target_idx]
        
        vector_to_target = steering_target_grid - est_pos_grid
        ...
        progress_px = g_to_s_func(progress_target_grid)
        steering_px = g_to_s_func(steering_target_grid)
        pygame.draw.circle(screen, GREEN, progress_px, 10, 2) # Progress target
        pygame.draw.line(screen, GREEN, g_to_s_func(est_pos_grid), steering_px, 2) # Steering target line
        ...
        text5 = hud_font.render(f"Target: {desired_speed_gups:.1f} gu/s", True, HUD_TEXT)
