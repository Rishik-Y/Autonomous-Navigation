...
def run_simulation():
    ...
    waypoints_filepath = r'C:\Users\DAIICT A\Downloads\Sub\Git\Autonomous-Navigation\Map_code8\waypoints.pkl'
    cache_filename = r'C:\Users\DAIICT A\Downloads\Sub\Git\Autonomous-Navigation\Map_code8\map_cache.pkl'
    ...
    def trace_back_and_patch(current_node, visited):
        ...
        # Definition of Spur Node: Not a Hub. 
        # Heuristic: Hubs usually have >2 edges connected or are named 'hub'.
        # Simple heuristic: If parent has <= 2 outgoing edges (one was to us, maybe one to somewhere else?)
        # Better: If parent is NOT in VISUAL_ROAD_CHAINS as an intersection? Hard to check.
        # Let's stop at explicit Hubs or Nodes with > 2 neighbors.
        
        parent_out_degree = outgoing_counts.get(parent, 0)
        parent_in_degree = len(incoming_map.get(parent, []))
        
        # If it looks like a simple road segment (1 in, 1 out OR 1 in, 2 out for splitting spur), keep going back.
        # If it looks like a Hub (many connections), stop.
