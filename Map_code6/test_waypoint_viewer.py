...
import math
import heapq
import random # Still needed for random choices if you want to uncomment later
import map_data
...
def run_viewer():
    ...
    pygame.display.set_caption("Waypoint Viewer (Fixed Path)")
    ...
    # --- Generate a single, fixed path ---
    start_node_name = "main_hub" # Fixed start node
    goal_node_name = "e_hub"     # Fixed goal node
    ...
    while running:
        ...
        # --- HUD ---
        hud_texts = [
            "Waypoint Viewer (Fixed Path) | Pan: Left-Click+Drag | Zoom: Mouse Wheel",
