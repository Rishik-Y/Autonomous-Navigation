...
class Car:
    ...
    def get_reference_trajectory(self, N, dt):
        ...
        for i in range(N + 1):
            ...
            # Calculate Normal for Swing
            # Tangent
            p_f = self.path.point_at(s_sim + 0.5)
            p_b = self.path.point_at(s_sim - 0.5)
            dx, dy = p_f[0] - p_b[0], p_f[1] - p_b[1]
            ...
            # Apply Swing
            p_swing_x = p[0] + nx * swing_val
            p_swing_y = p[1] + ny * swing_val
            
            # Heading (Tangent of the MODIFIED path ideally, but path tangent is close enough for small swings)
            # To be precise, we should look ahead on the swing path, but that's complex.
            # Use path tangent.
